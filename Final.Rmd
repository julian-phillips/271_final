...
We also will create three new binary variables, crimeRate_zero which indicates a very low crime rate and older neighborhood which indicates if 90% or greater of the houses was built before 1950.  Finally we have newerneighborhood which indicates if 25% or less of the houses were built before 1950.
...
ggplot(data, aes(newerneighborhood, homeValue)) + geom_boxplot() +  scale_y_continuous(name = "Home Value", labels = comma) 
...
lm = lm(homeValue ~ crimeRate_pc+crimeRate_zero+olderneighborhood +newerneighborhood +withWater+ageHouse+distanceToCity+pctLowIncome+pollutionIndex+nBedRooms, data=data)
