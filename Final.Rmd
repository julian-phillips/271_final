...
##I thought when the tail was on the right, it is a right skew - Yes it is, adjusted. 
 -----------------
...
Pollutionindex is defied as _scaled between 0 and 100, with 0 being the best and 100 being the worst (i.e. uninhabitable)_
...
The bunching that is concerning is eveident in the scatterplot.  There does seem to be a negative correlation between pollution index and home value.  The histrogram shows a right skew, but the greatest value does have many records.
...
Finally! A normally distributed variable.  This one is also positively correlated as well.  This will likely be one of the most useful of the prediction variables. Its range of 1.6 to 6.8 matches with what we know about bedrooms in hours too.
...
Discard 8 records that are categorical.
...
```{r}
lm = lm(homeValue ~ crimeRate_pc+withWater+ageHouse+distanceToCity+pctLowIncome+pollutionIndex+nBedRooms, data=data)
...
In bothmodels, withwater and ageHouse are far less significant than the other 5 variables.  I will remove them and recreate the models.
